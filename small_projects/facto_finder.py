...
# Use the factor finder to discover some new prime numbers! 
# (Hint: Prime numbers always end with an odd number that isn’t 5.)


import math, sys

print('Welcome to the prime number finder!')
print(''' A number's factors are two numbers that, when multiplied with each
other, produce the number. For example, 2 x 13 = 26, so 2 and 13 are
factors of 26. 1 x 26 = 26, so 1 and 26 are also factors of 26. We say that 26 has four factors: 1, 2, 13, and 26.
...
while True:
    print('Enter a positive whole number to factonr or "quit" to exit:')
    response = input('>>')
